...
class Player(object):
    ...
    def start(self, merge=False):
        # TODO: Restart player automatically in an attempt to keep playing
        # from the same position.  This would help in cases where the player
        # window is closed or the system restarts.
        ...
        if not get_property('mpv-version'):
            dirname = os.path.dirname(self.socket)
            os.makedirs(dirname, 0o775, True)
